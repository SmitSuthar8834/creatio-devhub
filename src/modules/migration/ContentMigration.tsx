  ...
  return <div className="grid gap-2">{rows.map((row) => {
    ...
    return (
      <Collapsible key={row.entity} className="rounded-lg border p-3">
        <div className="flex items-center gap-2"><strong className="font-mono text-sm">{row.entity}</strong><Badge variant="secondary">{row.inserted} inserted</Badge>{row.updated > 0 && <Badge>{row.updated} updated</Badge>}<span className="text-xs text-muted-foreground">{row.skipped} skipped{row.notSelected ? ` · ${row.notSelected} not selected` : ""}</span>
...
export default function ContentMigration() {
  ...
  /** Records the user can pick: missing ones, plus existing ones when overwriting. */
  const selectableOf = (entity: string) => overwrite.has(entity) ? (picks[entity] ?? []) : missingOf(entity);
  const isChosen = (entity: string, pick: ContentRecordPick) => chosen[entity]?.has(pick.id) ?? !pick.existsInTarget;
  ...
  const chooseAll = (entity: string, all: boolean) => setChosen((prev) => ({
    ...prev,
    [entity]: all ? new Set(selectableOf(entity).map((pick) => pick.id)) : new Set<string>(),
  }));
  const toggleOverwrite = (entity: string) => setOverwrite((prev) => {
    const next = new Set(prev);
    if (next.has(entity)) {
      next.delete(entity);
      // Drop any now-hidden existing records from the selection.
      setChosen((state) => {
        if (!state[entity]) return state;
        const existing = new Set(missingOf(entity).map((pick) => pick.id));
        return { ...state, [entity]: new Set([...state[entity]!].filter((id) => existing.has(id))) };
      });
    } else {
      next.add(entity);
    }
  ...
  return (
    <div className="relative grid gap-4 pt-2">
      {initializing && <LoadingOverlay label="Loading environments…" />}
      ...
      <div className="flex flex-wrap gap-2"><Button disabled={same || !!busy} onClick={() => run("Analyzing…", () => analyzeContent(source, target), setReport)}>{busy === "Analyzing…" ? <><Spinner data-icon="inline-start" />{busy}</> : "Analyze"}</Button><Button variant="outline" disabled={same || !!busy} onClick={() => run("Verifying…", () => verifyContent(source, target), setReport)}>{busy === "Verifying…" ? <><Spinner data-icon="inline-start" />{busy}</> : "Verify"}</Button></div>
      ...
      <div className="grid gap-3 rounded-lg border p-4"><div><h2 className="font-medium">Content records</h2><p className="text-sm text-muted-foreground">Parents are always copied before their dependents. Existing IDs are skipped by default — turn on <em>Overwrite</em> under Campaign or Bulk&nbsp;Email to update records already on the target instead. References to missing lookup rows are matched by name or cleared, and every change is reported.</p></div><div className="flex flex-wrap gap-4">{BASE.map((entity) => <Label key={entity} className="flex items-center gap-2 font-normal"><Checkbox checked={selected.has(entity)} onCheckedChange={() => toggle(entity)} />{entity}</Label>)}</div>
            ...
            <Collapsible key={entity} className="rounded-lg border" onOpenChange={(open) => open && loadPicks(entity)}>
              <CollapsibleTrigger asChild>
                <Button variant="ghost" className="w-full justify-between font-normal">
                  ...
                  <span className="flex items-center gap-2 text-xs text-muted-foreground">{picks[entity] ? `${chosenCount(entity)} of ${selectable.length} ${isOverwrite ? "" : "missing "}selected` : "all missing records"}<ChevronDown /></span>
                </Button>
              </CollapsibleTrigger>
              <CollapsibleContent className="grid gap-1 border-t p-3">
                {pickErrors[entity] && <ErrorNote error={pickErrors[entity]} />}
                {!picks[entity] && !pickErrors[entity] && <p className="flex items-center gap-2 text-sm text-muted-foreground"><Spinner />Loading records…</p>}
                {picks[entity] && <>
                  <Label className="mb-1 flex items-center gap-2 font-normal">
                    <Switch checked={isOverwrite} onCheckedChange={() => toggleOverwrite(entity)} />
                    <span className="text-sm">Overwrite records already on the target <span className="text-muted-foreground">(update them from the source)</span></span>
                  </Label>
                  <div className="mb-1 flex gap-2"><Button size="sm" variant="outline" onClick={() => chooseAll(entity, true)}>{isOverwrite ? "All" : "All missing"}</Button><Button size="sm" variant="outline" onClick={() => chooseAll(entity, false)}>None</Button></div>
                  ...
                  {picks[entity].map((pick) => (
                    <Label key={pick.id} className="flex items-center gap-2 py-0.5 font-normal">
                      <Checkbox disabled={pick.existsInTarget && !isOverwrite} checked={isChosen(entity, pick)} onCheckedChange={() => toggleRecord(entity, pick.id)} />
                      <span className="min-w-0 truncate text-sm">{pick.name || pick.id}</span>
                      {pick.existsInTarget && <Badge variant={isOverwrite && isChosen(entity, pick) ? "default" : "secondary"}>{isOverwrite && isChosen(entity, pick) ? "overwrite" : "on target"}</Badge>}
                    </Label>
                  ))}
                </>}
              </CollapsibleContent>
